Partial Public Class ucKabaddi
    Inherits ucNGBase
    ...
    Private Sub cmdPenaltyUpdate_Click(sender As Object, e As EventArgs) Handles cmdPenaltyUpdate.Click
        ...
        For iii = 1 To TeamStatus1.Value
            CasparDevice.SendString("call " & g_int_ChannelNumber & "-" & SafeInt(cmblayergames.Text) & " " & """" & "setCircleColor('ccgpenaltycircle" & iii & "'," & "'" & ColorTranslator.ToHtml(Color.LightGreen) & "')" & """")
        Next
        For iii = TeamStatus1.Value + 1 To 7
            CasparDevice.SendString("call " & g_int_ChannelNumber & "-" & SafeInt(cmblayergames.Text) & " " & """" & "setCircleColor('ccgpenaltycircle" & iii & "'," & "'" & ColorTranslator.ToHtml(Color.Red) & "')" & """")
        Next
        For iii = 1 To TeamStatus2.Value
            CasparDevice.SendString("call " & g_int_ChannelNumber & "-" & SafeInt(cmblayergames.Text) & " " & """" & "setCircleColor('ccgpenaltycircle" & iii + 7 & "'," & "'" & ColorTranslator.ToHtml(Color.LightGreen) & "')" & """")
        Next
        For iii = TeamStatus2.Value + 1 To 7
            CasparDevice.SendString("call " & g_int_ChannelNumber & "-" & SafeInt(cmblayergames.Text) & " " & """" & "setCircleColor('ccgpenaltycircle" & iii + 7 & "'," & "'" & ColorTranslator.ToHtml(Color.Red) & "')" & """")
        Next
    ...
    Private Sub cmd30cueSecondTimer_Click(sender As Object, e As EventArgs)
        ...
        CasparDevice.Channels(g_int_ChannelNumber - 1).CG.Add(SafeInt(cmblayergames.Text) + 2, SafeInt(cmblayergames.Text) + 2, "ng2022/template/kabaddi/30sectimer1", True, CasparCGDataCollection.ToAMCPEscapedXml)
    End Sub
    Private Sub cmdcue30SecondTimer2_Click(sender As Object, e As EventArgs)
        On Error Resume Next
        CasparCGDataCollection.Clear()
        CasparCGDataCollection.SetData("ccgf0", RaidTime.Text)
        CasparDevice.Channels(g_int_ChannelNumber - 1).CG.Add(SafeInt(cmblayergames.Text) + 2, SafeInt(cmblayergames.Text) + 2, "ng2022/template/kabaddi/30sectimer2", True, CasparCGDataCollection.ToAMCPEscapedXml)
